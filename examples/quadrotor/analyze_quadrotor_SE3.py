...
def get_model():
    model =  SE3HamNODE(device=device, udim = 5,pretrain = False).to(device)
    path = f'{THIS_DIR}/jackal-se3ham-rk4-5p.tar'
    model.load_state_dict(torch.load(path, map_location=device))
    path = f'{THIS_DIR}/jackal-se3ham-rk4-5p-stats.pkl'
...
if __name__ == "__main__":
    ...
    # for i in range(len(stats['test_loss'])):
    #     if stats['test_loss'][i] > 10:
    #         stats['test_loss'][i] = 0.2
    # stats['test_loss'][stats['test_loss'] > 10] = 0.25
    # print(f"train loss : {stats['train_loss']}")
    # print("----------------------------")
    # print(f"test loss : {stats['test_loss']}")
    ...
    plt.savefig('examples/quadrotor/png/loss_log.png', bbox_inches='tight', pad_inches=0.1)
    plt.show()
    ...
    plt.yticks(fontsize=fontsize_ticks)
    plt.legend(fontsize=fontsize)
    plt.savefig('examples/quadrotor/png/SO3_constraints_test.png', bbox_inches='tight', pad_inches=0.1)
    plt.show()
    ...
    plt.yticks(fontsize=fontsize_ticks)
    plt.legend(fontsize=fontsize)
    plt.savefig('examples/quadrotor/png/V_x.png', bbox_inches='tight', pad_inches=0.1)
    plt.show()
    ...
    plt.yticks(fontsize=fontsize_ticks)
    plt.legend(fontsize=fontsize)
    plt.savefig('examples/quadrotor/png/M1_x_all.png', bbox_inches='tight', pad_inches=0.1)
    plt.show()
    ...
    plt.savefig('examples/quadrotor/png/M2_x_all.png', bbox_inches='tight')
    plt.show()
    ...
    plt.yticks(fontsize=fontsize_ticks)
    plt.legend(fontsize=fontsize)
    plt.savefig('examples/quadrotor/png/g_v_x.png', bbox_inches='tight', pad_inches=0.1)
    plt.show()
    ...
    plt.yticks(fontsize=fontsize_ticks)
    plt.legend(fontsize=fontsize)
    plt.savefig('examples/quadrotor/png/g_w_x.png', bbox_inches='tight', pad_inches=0.1)
    plt.show()
