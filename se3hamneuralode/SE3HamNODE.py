...
class SE3HamNODE(torch.nn.Module):
    def __init__(self, device=None, pretrain = True, M_net1  = None, M_net2 = None, V_net = None, g_net = None, udim = 4):
        ...
        self.udim = udim
        if M_net1 is None:
            self.M_net1 = PSD(self.xdim, 100, self.linveldim, init_gain=init_gain).to(device)
        else:
            self.M_net1 = M_net1
        if M_net2 is None:
            self.M_net2 = PSD(self.Rdim, 100, self.twistdim - self.linveldim, init_gain=init_gain).to(device)
        else:
            self.M_net2 = M_net2
        if V_net is None:
            self.V_net = MLP(self.posedim, 100, 1, init_gain=init_gain).to(device)
        else:
            self.V_net = V_net
        if g_net is None:
            self.g_net = MatrixNet(self.posedim, 100, self.twistdim*self.udim, shape=(self.twistdim,self.udim), init_gain=init_gain).to(device)
        else:
            self.g_net = g_net
        self.device = device
        self.nfe = 0
